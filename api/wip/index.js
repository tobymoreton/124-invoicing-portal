...
module.exports = async function (context, req) {
  ...
  try {
    ...
    const items   = await fetchAllWIP(token, context);
    ...
    context.res = {
      ...
      headers: {
        ...
        'X-Api-Build': 'S103-exclude-nonbillable',
      },
...
async function fetchAllWIP(token, context) {
  // Server-side filter stays "not yet billed" ONLY.
  //
  // 🔴 S103 (2026-08-05) — non-billable rows are now excluded, but CLIENT-side, never in the
  // Graph $filter. Two reasons, both learned the hard way on this tenant:
  //   1. `fields/Billable_x003f_ eq true` would drop rows where the column is ABSENT, and
  //      /api/caseactions deliberately omits Billable_x003f_ on POST (boolean writes are
  //      unreliable here), so every portal-created entry reads as undefined, not false.
  //      Dropping those would hide real WIP — the same trap /api/coa documented on
  //      2026-08-03. Only an EXPLICIT false is excluded below.
  //   2. The old comment here ("Billable flag unreliable — many valid WIP entries have
  //      Billable=false") predates that finding, which established the flag is clean:
  //      a 23-true / 90-false split on 1736520 was correct, not corruption.
  //
  // Why exclude at all: PA555/PA555.1 now rate non-billable entries too (so the Case Actions
  // list reads sensibly and an entry flipped to billable later is already rated). Measured
  // 05/08/2026: 2,033 non-billable unbilled rows, 315 adrift from their case rate, many at
  // £0. Rating them while still counting them here would have added £7,517.50 of explicitly
  // NON-billable time to the WIP dashboards overnight.
  //
  // This also settles a long-standing disagreement: case.html's Unbilled WIP box already
  // excluded non-billable (`Billable_x003f_ !== false`). The dashboards fed by this function
  ...
  while (url) {
    ...
    fetched += raw.length;
    // EXPLICIT false only — undefined/absent stays IN (see the note above).
    const keep  = raw.filter(it => (it.fields || {})['Billable_x003f_'] !== false);
    nonBillableExcluded += raw.length - keep.length;
    all = all.concat(keep.map(normalise));
    url = page['@odata.nextLink'] || null;
  }

  // Watchdog. The response shape here is a bare array (every caller depends on that), so the
  // count cannot ride along in the body — log it. If this ever reads 0 against a few thousand
  // fetched rows, the boolean has stopped coming back from Graph and this filter is silently
  // keeping everything; if it ever approaches `fetched`, it is silently dropping everything.
  // Expected ~2,000 of ~4,000 as at 05/08/2026.
  if (context && context.log) {
    context.log(`P124 /api/wip — fetched ${fetched} unbilled, excluded ${nonBillableExcluded} non-billable, returning ${all.length}`);
  }
